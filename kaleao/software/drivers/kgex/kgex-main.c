...
{    
	...
	dma_write32(lp, DMA_RX_CR_OFFSET, cr & (~DMA_CR_RUNSTOP_MASK));        
        ...
        {
            dbg1("kgex_stop(): before netif_stop_queue()\n");    
...
//         tasklet_disable(&(lp->kgex_tx_poll));
        ...
        dbg1("kgex_stop(): before kgex_rings_free()\n");    
        kgex_rings_free(ndev);
        
        dbg1("kgex_stop(): before free_irq()\n");    
...
//	ndev->features |= NETIF_F_RXCSUM;
	...
	if (status) {
        ...
        {         
            ...
            if (status==1)
            {
                memcpy(ndev->dev_addr, exynos_2_mac_B_addr, ndev->addr_len); 
