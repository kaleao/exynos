...
#define KGEX_MAX_NET_DEVS   4

#define KGE_REGS_N  81
...
#define KGE_MTU			      1500 /* Max MTU of an Ethernet frame */
#define KGE_JUMBO_MTU		      9000 /* Max MTU of a jumbo Eth. frame */

#define KGE_MAX_FRAME_SIZE	 (KGE_MTU + KGE_HDR_SIZE + KGE_TRL_SIZE)
#define KGE_MAX_VLAN_FRAME_SIZE  (KGE_MTU + VLAN_ETH_HLEN + KGE_TRL_SIZE)
...
#define KGEX_NAPI_WEIGHT		64

#ifdef RX_ENABLE
	//#warning "--------> RX_ENABLE\n"
	#pragma message ( "--------> RX_ENABLE" )	
#endif

#ifdef TX_ENABLE
	//#warning "--------> TX_ENABLE\n"
	#pragma message ( "--------> TX_ENABLE" )
#endif
...
/* Workaround for AXI DMA interrupt clear bug. Status Register needs to be written many times 
   in order to get the required value. */
//#define BUG_AXI_DMA_TX_SR_CLEAR
//#define BUG_AXI_DMA_RX_SR_CLEAR

//Enable this if the Xilinx AXI DMA endpoint is 64-bit
//#define AXI_DMA_64

#ifdef AXI_DMA_64
    //#warning "--------> AXI DMA 64-bit"   
    #pragma message ( "--------> AXI DMA 64-bit" )
#else
    //#warning "--------> AXI DMA 32-bit"   
    #pragma message ( "--------> AXI DMA 32-bit" )
#endif


/* ===== Ethernet Interface IDs ==================================================================*/
#define  ETH_ID_NIC_A   0x0      /* Network FPGA NIC A */
#define  ETH_ID_NIC_B   0x1      /* Network FPGA NIC B */
#define  ETH_ID_NIC_C   0x2      /* Storage FPGA NIC C*/
#define  ETH_ID_NIC_D   0x3      /* Network FPGA Management NIC C */

/* ===== Base Physical Addresses =================================================================*/
#define BASE_NIC_A_DMA  0x1C100000   /* Network FPGA NIC A */
#define BASE_NIC_C_DMA  0x18100000   /* Storage FPGA NIC */
#define LEN_DMA_REGS    0x1000
...
#define DMA_DFT_TX_THRESHOLD	75
#define DMA_DFT_TX_WAITBOUND	254
#define DMA_DFT_RX_THRESHOLD	50
...
#define TX_RING_BYTES	(sizeof(struct dma_desc) * TR_RING_SIZE)
    ...
    struct dma_bd {
            ...
            uint32_t status;
            uint32_t app0;
            uint32_t rx_skb_virt_lo;   /*Modified for hybrid 64-bit 32-bit operation*/
            uint32_t rx_skb_virt_hi;   /*Modified for hybrid 64-bit 32-bit operation*/
            uint32_t app3;            
            uint32_t tx_skb_virt_lo;   /*Modified for hybrid 64-bit 32-bit operation*/
            uint32_t tx_skb_virt_hi;   /*Modified for hybrid 64-bit 32-bit operation*/
            uint32_t reserved5;
            uint32_t reserved6;
    };
#endif
...
struct net_private {
	...
	struct device *dev;
        ...
        struct net_device *ndev_nicB;
        struct net_device *dev_nicB;
        uint32_t id;
        uint32_t has_nicB;
	...
	struct tasklet_struct tx_poll_task;
        struct tasklet_struct rx_poll_task;                
...
#define WHOLE(lo, hi)	((uint64_t) (hi) << 32 | (lo)); 
    ...
    {
                    hw_write32(lp->regs, offset, val);
    }
    static inline uitn32_t nic_read32(struct net_private *lp, off_t offset)
    {
                    return ReadReg32(lp->regs, offset);
    }

    static inline void dma_write32(struct net_private *lp, off_t offset, uint32_t val)
    {    
...
#else
    static inline void nic_write32(struct net_private *lp, uint32_t offset, uint32_t val)
    {
                    hw_write32(lp->regs, offset, val);
    }
    ...
    static inline void dma_write32(struct net_private *lp, uint32_t offset, uint32_t val)
    {    
                    wmb();
                    hw_write32(lp->dma_regs, offset, val);		
                    wmb();
