...
int create_proc_eth10g(struct net_device *ndev)
{
	...
	if (lp->proc_eth10g==NULL){prerr("eth10g-eth: WARNING: not enough mem to create proc fs\n");return 1;}
	proc_lp = lp;
	
	lp->proc_eth10g->dir_main = proc_mkdir("eth10g-eth", 0);
	if (lp->proc_eth10g->dir_main==NULL){prerr("eth10g-eth: WARNING: no mem for main dir\n");return 1;}
	
	lp->proc_eth10g->dir_mac = proc_mkdir("mac", lp->proc_eth10g->dir_main);
	if (lp->proc_eth10g->dir_main==NULL){prerr("eth10g-eth: WARNING: no mem for mac dir\n");return 1;}

	lp->proc_eth10g->fops_frm_tx_ok = (struct file_operations *)kmalloc(sizeof(struct file_operations), GFP_KERNEL);
	if (lp->proc_eth10g->fops_frm_tx_ok == NULL){prerr("eth10g-eth: WARNING: no mem for fops_frm_tx_ok\n");return 1;}
	...
	if (lp->proc_eth10g->frm_tx_ok == NULL){prerr("eth10g-eth: WARNING: no mem for frm_tx_ok\n");return 1;}		

	lp->proc_eth10g->fops_rx_stats = (struct file_operations *)kmalloc(sizeof(struct file_operations), GFP_KERNEL);
	if (lp->proc_eth10g->fops_rx_stats == NULL){prerr("eth10g-eth: WARNING: no mem for fops_rx_stats\n");return 1;}
	...
	if (lp->proc_eth10g->rx_stats == NULL){prerr("eth10g-eth: WARNING: no mem for rx_stats\n");return 1;}		
	
	lp->proc_eth10g->fops_tx_stats = (struct file_operations *)kmalloc(sizeof(struct file_operations), GFP_KERNEL);
	if (lp->proc_eth10g->fops_tx_stats == NULL){prerr("eth10g-eth: WARNING: no mem for fops_tx_stats\n");return 1;}
	...
	if (lp->proc_eth10g->tx_stats == NULL){prerr("eth10g-eth: WARNING: no mem for tx_stats\n");return 1;}		
	
	lp->proc_eth10g->fops_mac_config = (struct file_operations *)kmalloc(sizeof(struct file_operations), GFP_KERNEL);
	if (lp->proc_eth10g->fops_mac_config == NULL){prerr("eth10g-eth: WARNING: no mem for fops_mac_config\n");return 1;}
	...
	if (lp->proc_eth10g->mac_config == NULL){prerr("eth10g-eth: WARNING: no mem for mac_config\n");return 1;}		
	
	lp->proc_eth10g->fops_rx_config = (struct file_operations *)kmalloc(sizeof(struct file_operations), GFP_KERNEL);
	if (lp->proc_eth10g->fops_rx_config == NULL){prerr("eth10g-eth: WARNING: no mem for fops_rx_config\n");return 1;}
	...
	if (lp->proc_eth10g->rx_config == NULL){prerr("eth10g-eth: WARNING: no mem for rx_config\n");return 1;}		

	lp->proc_eth10g->fops_tx_config = (struct file_operations *)kmalloc(sizeof(struct file_operations), GFP_KERNEL);
	if (lp->proc_eth10g->fops_tx_config == NULL){prerr("eth10g-eth: WARNING: no mem for fops_tx_config\n");return 1;}
	...
	if (lp->proc_eth10g->tx_config == NULL){prerr("eth10g-eth: WARNING: no mem for tx_config\n");return 1;}		
...
static int proc_show(struct seq_file *m, void *v)
{
	...
	if (m->private==NULL){seq_printf(m, "eth10g-eth: WARNING: show function is NULL\n");return  0;}
...
{	
	...
	if (proc_lp==NULL){seq_printf(m, "eurv-eth: WARNING: private network device is NULL\n");return 0;}
