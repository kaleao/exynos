...
void print_part_efi(block_dev_desc_t * dev_desc)
{
	...
	if (is_gpt_valid(dev_desc, GPT_PRIMARY_PARTITION_TABLE_LBA,
		...
		if (is_gpt_valid(dev_desc, (dev_desc->lba*DIV - 1),
				 gpt_head, &gpt_pte) != 1) {
	...
	for (i = 0; i < le32_to_cpu(gpt_head->num_partition_entries); i++) {
		...
		if (!strncmp(print_efiname(&gpt_pte[i]), "userdata", sizeof("userdata")))
			printf("%3d\t0x%16llx\t0x%16llx\t\"%s\"\n", (i + 1),
				le64_to_cpu(gpt_pte[i].starting_lba),
				le64_to_cpu(gpt_pte[i].ending_lba - FMP_USE_SIZE),
				...
				disk_partition_t * info)
{
	...
	info->start = (lbaint_t)(le64_to_cpu(gpt_pte[part - 1].starting_lba)*DIV);
	/* The ending LBA is inclusive, to calculate size, add 1 to it */
	info->size = ((lbaint_t)le64_to_cpu(gpt_pte[part - 1].ending_lba) + 1)*DIV
		     - info->start;
	info->blksz = dev_desc->blksz;

	sprintf((char *)info->name, "%s",
			print_efiname(&gpt_pte[part - 1]));
	if (!strncmp(info->name, "userdata",sizeof("userdata")))
		info->size = info->size - FMP_USE_SIZE;
		...
		gpt_header *gpt_h, gpt_entry *gpt_e)
{
	const int pte_blk_cnt = BLOCK_CNT((gpt_h->num_partition_entries
					   * sizeof(gpt_entry)), dev_desc)*DIV;
	u32 calc_crc32;
	u64 val;
	debug("max lba: %x\n", (u32) dev_desc->lba*DIV);
	/* Setup the Protective MBR */
	...
	if (dev_desc->block_write(dev_desc->dev, 1*DIV, 1, gpt_h) != 1)
		goto err;

	if (dev_desc->block_write(dev_desc->dev, 2*DIV, pte_blk_cnt, gpt_e)
	    != pte_blk_cnt)
		goto err;
	...
	if (dev_desc->block_write(dev_desc->dev,
				  (lbaint_t)(le64_to_cpu(gpt_h->my_lba)*DIV), 1,
				  gpt_h) != 1)
		goto err;
...
int gpt_fill_pte(block_dev_desc_t *dev_desc,gpt_header *gpt_h, gpt_entry *gpt_e,
		disk_partition_t *partitions, int parts)
{
	...
	for (i = 0; i < parts; i++) {
		...
		if (offset >= (last_usable_lba)) {
			printf("Partitions layout exceds disk size %lld %lld %d \n",offset,last_usable_lba,i);
			return -1;
		}
		...
		char *str_guid, int parts_count)
{
	...
	gpt_h->first_usable_lba = cpu_to_le64((32 + 2*DIV)/DIV);
	gpt_h->last_usable_lba = cpu_to_le64(dev_desc->lba)-(32+2*DIV)/DIV;
			...
			gpt_header *pgpt_head, gpt_entry **pgpt_pte)
{
	...
	if (dev_desc->block_read(dev_desc->dev, (lbaint_t)lba*DIV, 1, pgpt_head)
			!= 1) {
					 ...
					 gpt_header * pgpt_head)
{
	...
	blk_cnt = BLOCK_CNT(count, dev_desc)*DIV;
	if (dev_desc->block_read (dev_desc->dev,
		(lbaint_t)le64_to_cpu(pgpt_head->partition_entry_lba)*DIV,
