...
#endif /* CONFIG_BOOTCOUNT_LIMIT */
#ifdef FAST_ENTER_FASTBOOT
	usb_status = check_usb_gpio(); 
	if(usb_status && (readl(second_boot_info_base)!=2))
	{//usb connect enter the fastboot
		printf("connected the usb enter fastboot!\n");
		fastboot_usb_flag = 1;
