	...
	if (*newp == '<') {
		...
		while (stridx < count) {
			size_t length = strlen(newp) + 1;
				//strcat(data," "); /*Deleted by Huangwenjian for fdt set string property 20150612*/
				strcat(data,newp);
/*Added by Huangwenjian for fdt set string property 20150612 begin*/				
			if (stridx < count -1)
				strcat(data," ");
/*Added by Huangwenjian for fdt set string property 20150612 end*/					
			//strcpy(data, newp);
			//data += length;
			*len += length;
			newp = newval[++stridx];
...
static int fdt_print(const char *pathp, char *prop, int depth)
{
	...
	if (prop) {
		...
		if (len == 0) {
			...
			printf("\n");
                    if(!apdebug_changed) {
                        char * p = strstr(nodep, "apdebug_onoff=");
                        int len = 0;
                        if(p) {
                            p += strlen("apdebug_onoff=");
                            if(!memcmp(p, "off", strlen("off")))
                                len = strlen("off");
                            else if(!memcmp(p, "off", strlen("on")))
                                len = strlen("on");
                            if(len) {
                                memset(result_apdebug, 0, sizeof(result_apdebug));
                                strncpy(result_apdebug, p, len);
                            }
                        }
                    }
			return 0;
