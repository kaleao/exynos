...
static int fixup_memory_node(void *blob)
{
    ...
    if (dram_size == 0xC0000000)
            return fdt_fixup_memory_banks(blob, start, size, 12);
    else if (dram_size == 0x100000000)
            return fdt_fixup_memory_banks(blob, start, size, 16);
    else
...
{
	printf("\nStarting kernel ...\n\n");
	printf("Forcing SD OFF!!!\n");
	*((volatile uint32_t *)0x15560004) = 0x0;
	*((volatile uint32_t *)0x15560000) = 0x1;
	*((volatile uint32_t *)0x14870024) = (*((volatile uint32_t *)0x14870024) & 0xFFFFFFF5) | 0x8;
...
int do_bootm_linux(int flag, int argc, char *argv[], bootm_headers_t *images)
{
	...
	boot_prep_linux(images);
	arch_preboot_os(images->rd_start,images->rd_end);
	boot_jump_linux(images);
	return 0;
}
