...
//#define CONFIG_SYS_MMC_MAX_BLK_COUNT 512
//#define CONFIG_SYS_MMC_MAX_BLK_COUNT 0x40000
#endif
...
static ulong mmc_bread(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
	...
	//printf("BABIS: Resetting!\n");
	//*((volatile uint32_t *) 0x10580000) = 0x10;			
	//*((volatile uint32_t *) 0x10580004) = *((volatile uint32_t *) 0x10580004) | 0x2;
...
//		udelay(1000);
		...
		{			
			...
			if (!mmc) {
	                        printf("BABIS: no mmc device at slot %x\n", dev_num);
	                        return 0;
        	        }
...
//	err = mmc_send_cmd(mmc, &cmd, NULL);
 ...
 retry_scr:
	...
	//printf("BABIS:  Send dummy CMD 51 (Read Control Register)\n");
//BABIS: Removed cmd 51 for custom SD Slave
//	err = mmc_send_cmd(mmc, &cmd, &data);
	err = 0;
	if(err)
	{
		printf("BABIS: CMD 51 returned error but I continue!\n");
	}
	err = 0;
...
//BABIS: Added custom configuration for SD slave
//	mmc->scr[0] = __be32_to_cpu(scr[0]);
//	mmc->scr[0] = 0x02050000;
	mmc->scr[0] = 0x00050000;	//Version 1
...
int mmc_init(struct mmc *mmc)
{
	...
	if (mmc->has_init)
		return 0;
	//printf("BABIS: mmc_init: Calling mmc->init\n");
	err = mmc->init(mmc);

	if (err)
		return err;
	
	//printf("BABIS: mmc_init: Calling set_bus_width\n");
	mmc_set_bus_width(mmc, 1);

	//printf("BABIS: mmc_init: mmc_set_clock\n");
	mmc_set_clock(mmc, 1);
	...
	//printf("BABIS: mmc_init: Calling go_idle\n");
	err = mmc_go_idle(mmc);
	...
	//printf("BABIS: mmc_init: Calling send_if_cmd Version 2\n");
	/* Test for SD version 2 */
	err = mmc_send_if_cond(mmc);

	
	//printf("BABIS: mmc_init: Calling get Operating condition\n");
	/* Now try to get the SD card's operating condition */
	err = sd_send_op_cond(mmc);

	
	//printf("BABIS: mmc_init: mmc TIMEOUT\n");
	...
	//printf("BABIS: Set maximum block count per command: %llu\n", CONFIG_SYS_MMC_MAX_BLK_COUNT);
