...
static u32 fastboot_receiving_data = 0;
static u32 fastboot_receive_done = 0;
...
int fboot_usb_handle_ep_out_xfer_complete(void)
{
	u32 usRxCnt;
	usbdev3_trb_ctrl_t usbdev3_trb_ctrl;
	if (fastboot_receiving_data) 
	{ 	fastboot_receive_done = 1;
	 	return 1;
	}
...
int fastboot_receive_data(char *buf, u32 size)
{
	u32 usCapTrbBufSiz;
	u32 uLastBufSize;
	u32 i = 0;
	...
	oUsbDev3.m_pDownPt = buf;
	{
		usCapTrbBufSiz = TRB_BUF_SIZ_LIMIT/oUsbDev3.m_uBulkEPMaxPktSize*oUsbDev3.m_uBulkEPMaxPktSize;

		g_uCntOfBulkOutTrb = size/usCapTrbBufSiz;
		if (size%usCapTrbBufSiz != 0) {
			g_uCntOfBulkOutTrb++;
		}
		...
		usbdev3_trb_ctrl.b.chn = 0;
		usbdev3_trb_ctrl.b.ioc = 1;
		uLastBufSize = size - usCapTrbBufSiz*i;
	...
	fastboot_receiving_data = 1;
	fastboot_receive_done = 0;

	while (!fastboot_receive_done) {
		fastboot_poll();
	}

	fastboot_receiving_data = 0;
	exynos_usb_free((u32)g_pBulkOutTrbArray_Base);
