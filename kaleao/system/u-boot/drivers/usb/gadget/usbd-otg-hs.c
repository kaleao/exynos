...
#include "fastboot.c"
int check_mux_status(u32 *regs,int offset)
{
	u32 val;
	...
	do {
		--timeout;
		if(!timeout) {
			printf("failed to enable user mux!\n");
			return FALSE;
		}
		val = readl(regs);
		val &= (0x4) << offset;
...
#elif defined(CONFIG_CPU_EXYNOS3250) || defined(CONFIG_CPU_EXYNOS3472)
	writel(0x0, USB_CFG_REG);
	writel(0x7f80, S5P_OTG_PHYPWR);
	writel(0x215, S5P_OTG_PHYCLK);
#elif defined(CONFIG_CPU_EXYNOS7580)
	writel(0x0, USB_CFG_REG);
	writel(0x10, EXYNOS_PHYUTMI);
	writel(0xe8, EXYNOS_PHYCLKRST);
#endif

#if defined(CONFIG_CPU_EXYNOS7580)
	writel(0x80150431, EXYNOS_HOSTPHYCTRL0);
	udelay(10);
	writel(0x00150430, EXYNOS_HOSTPHYCTRL0);
	udelay(50);
	/* Enable User MUX */
	if(check_mux_status(CLK_MUX_STAT_FSYS2,12)) {
		writel(0x1000,CLK_MUX_SEL_FSYS2);
	} else {
...
#elif defined(CONFIG_CPU_EXYNOS7580)
	writel(0x5f,EXYNOS_PHYUTMI);
	writel(0xe9,EXYNOS_PHYCLKRST);
	writel(0x80150471,EXYNOS_HOSTPHYCTRL0);
	writel(0x0,EXYNOS_OTGLINKCTRL);
#else
