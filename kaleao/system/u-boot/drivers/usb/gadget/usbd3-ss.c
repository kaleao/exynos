...
static void exynos_usb_set_descriptor_tlb(void)
{
	...
	if(g_dnw_version >= DNW_v10) {
		oUsbDev3.m_oDesc.oDescDevice.idProductL=0x09;
		oUsbDev3.m_oDesc.oDescDevice.idProductH=0x29;
	}else
...
#ifdef CONFIG_DNW_VERSION
	if(g_dnw_version >= DNW_v05) // fastboot_over_dnw)
		oUsbDev3.m_oDesc.oDescDevice.iSerialNumber=0x3;
	else
#endif /* CONFIG_DNW_VERSION */
...
static void exynos_usb_handle_connect_done_int(void)
{
	...
	printf("%s, g_dnw_version:%d\n", __func__, g_dnw_version); // test
...
#ifdef CONFIG_DNW_VERSION
				else if(DNW_v10 == g_dnw_version)  // dnw v.1.0
			...
			switch (oUsbDev3.m_oDeviceRequest.wValue_H)
			{
				...
				case STRING_DESCRIPTOR :
					...
					switch(oUsbDev3.m_oDeviceRequest.wValue_L)
					{
						...
						case 3:
#ifdef CONFIG_DNW_VERSION
							if (DNW_v05 <= g_dnw_version) /*fastboot_over_dnw*/{
								printf("%s, send string3:serial \n", __func__);
				...
				case BOS :
				{
					...
					if (oUsbDev3.m_uDeviceRequestLength == BOS_DESC_SIZE)
						...
						printf("%s, USB 2.0 does not support BOS\n", __func__);
					}
				}
					break;
			}
