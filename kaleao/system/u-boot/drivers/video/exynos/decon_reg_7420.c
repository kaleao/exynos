...
void decon_reg_set_porch(u32 id, int dsi_idx, struct decon_lcd *info)
{
	...
	if (id == 0)
		val = VIDTCON4_LINEVAL(info->yres - 1) | VIDTCON4_HOZVAL((info->xres >> 1) - 1);
	else
#endif
		...
		struct decon_lcd *lcd_info)
{
	...
	if (dsi_mode == DSI_MODE_DUAL) {
		...
		decon_write_mask(id, FRAMEFIFO_REG9, ~0, 0x1);
		decon_write_mask(id, FRAMEFIFO_REG10, ~0, lcd_info->xres >> 1);
	}
