...
#define REG_CLK_MUX_SEL_DISP2			(0x14870208)
#define REG_CLK_MUX_SEL_DISP4			(0x14870210)
#define PHYCLK_DSI_LINK0_RXCLKESC0_USER_SEL	(1<<20)
#define PHYCLK_DSI_LINK0_BITCLKDIV8_USER_SEL	(1<<16)
...
void dsim_reg_set_pll_freq(u32 id, u32 p, u32 m, u32 s)
{
	...
	dsim_write_mask(id, DSIM_PLLCTRL1, 0x005003fa, ~0);
	dsim_write_mask(id, DSIM_PLLCTRL_M1, 0x40000000, ~0);
}
...
u32 dsim_reg_is_pll_stable(u32 id)
{
	...
	if (val & DSIM_STATUS_PLL_STABLE) {
		val = readl(REG_CLK_MUX_SEL_DISP2);
		val |= PHYCLK_DSI_LINK0_RXCLKESC0_USER_SEL;
		val |= PHYCLK_DSI_LINK0_BITCLKDIV8_USER_SEL;
		writel(val, REG_CLK_MUX_SEL_DISP2);
		writel(0, REG_CLK_MUX_SEL_DISP4);
		return 1;
	}
	return 0;
}
