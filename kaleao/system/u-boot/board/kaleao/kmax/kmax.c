...
//	display_tmu_info();
	...
	//BABIS: Write 0 to GPIO PIN XEINT_1
	/**((volatile uint32_t *)0x10580000) = 0x10;	
	*((volatile uint32_t *)0x10580004) = *((volatile uint32_t *)0x10580004) & 0xFFFFFFFE;*/

	*((volatile uint32_t *) 0x10580000) = 0x10;			
	*((volatile uint32_t *) 0x10580004) = *((volatile uint32_t *) 0x10580004) & ~0x2;
	
	//BABIS: Write 01 to GPIO PIN XGPIO_2, XGPIO_4
	*((volatile uint32_t *)0x14870020) = 0x1010;	
	*((volatile uint32_t *)0x14870024) = (*((volatile uint32_t *)0x14870024) & 0xFFFFFFF5) | 0x00000002;
...
static void configure_auto_boot(unsigned int om_status)
{
	...
	setenv("boot_recovery", CONFIG_BOOT_RECOVERY);
	...
	else  
	{
		...
		if ((readl(&pmu->sysip_dat0)) != CONFIG_FACTORY_RESET_MODE)
		{
			if(om_status == BOOT_UFS){	
				//BABIS: GPIO!
				//BABIS: Write 0 to GPIO PIN XEINT_0
				*((volatile uint32_t *)0x10580000) = 0x10;
				
				printf("\n\n\n\n\n\n\n\n\n\nBABIS: CHECKING %X\n\n\n\n\n\n\n\n\n\n", (*((volatile uint32_t *)0x10580004)));
				if((*((volatile uint32_t *)0x10580004)) & 0x1)
				{
					setenv("bootcmd", CONFIG_BOOT_RECOVERY);
				}else
				{
