...
void pmic_enable_peric_dev(void)
{
    ...
    /*--------------------------------------------->*/	
    /* enable LDO18 : VCC_1.8V_PERI */
	IIC0_ERead(S2MPS15_ADDR, LDO18_CTRL, &ldo_ctrl);
	ldo_ctrl |= OUTPUT_PWREN_ON;
	IIC0_EWrite(S2MPS15_ADDR, LDO18_CTRL, ldo_ctrl);

	/* enable LDO21 : VCC_3.3V_PERI */
	IIC0_ERead(S2MPS15_ADDR, LDO21_CTRL, &ldo_ctrl);
	ldo_ctrl |= OUTPUT_PWREN_ON;
	IIC0_EWrite(S2MPS15_ADDR, LDO21_CTRL, ldo_ctrl);			
#ifdef LCD_TOUCHPANEL_COMPATIBLE
	IIC0_ERead(S2MPS15_ADDR, LDO19_CTRL, &ldo_ctrl);
	ldo_ctrl |= OUTPUT_PWREN_ON;
	IIC0_EWrite(S2MPS15_ADDR, LDO19_CTRL, ldo_ctrl);	

	IIC0_ERead(S2MPS15_ADDR, LDO23_CTRL, &ldo_ctrl);
	ldo_ctrl |= OUTPUT_PWREN_ON;
	IIC0_EWrite(S2MPS15_ADDR, LDO23_CTRL, ldo_ctrl);		
#endif
}
