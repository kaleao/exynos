...
static inline u64 exynos_smc_read(u64 cmd)
{
	register u64 reg0 __asm__("x0") = cmd;
...
unsigned int load_uboot_image(u32 boot_device)
{
	...
	return exynos_smc(SMC_CMD_LOAD_UBOOT,
			(u64)boot_device, (u64)info_image, 0);
#else
	if (boot_device == SDMMC_CH2) {
...
unsigned int coldboot(u32 boot_device)
{
	...
	return exynos_smc(SMC_CMD_COLDBOOT,
			(u64)boot_device, (u64)info_image, CONFIG_SYS_TEXT_BASE);
#else
	return exynos_smc(SMC_CMD_COLDBOOT,
			(u64)boot_device, (u64)info_image, CONFIG_PHY_IRAM_NS_BASE);
#endif
#else
