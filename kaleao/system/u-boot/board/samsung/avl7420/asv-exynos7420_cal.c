...
static void asv_table_init(void)
{
	...
	if (gasv_table_info.dvfs_asv_table_version == 12 || gasv_table_info.dvfs_asv_table_version == 14) {
...
u32 cal_get_volt(u32 id, s32 level)
{
	...
	if (table_ver == 0) {
		...
		p_table = ((id == SYSC_DVFS_BIG) ? volt_table_big_asv_v12[idx] :
				...
				NULL);
	} else if(table_ver == 13) {
		p_table = ((id == SYSC_DVFS_BIG) ? volt_table_big_asv_v13[idx] :
				(id == SYSC_DVFS_LIT) ? volt_table_lit_asv_v13[idx] :
				(id == SYSC_DVFS_G3D) ? volt_table_g3d_asv_v13[idx] :
				(id == SYSC_DVFS_MIF) ? volt_table_mif_asv_v13[idx] :
				(id == SYSC_DVFS_INT) ? volt_table_int_asv_v13[idx] :
				(id == SYSC_DVFS_CAM) ? volt_table_cam_asv_v13[idx] :
				NULL);
...
u32 get_asv_table_version(void)
{
	return (gasv_table_info.dvfs_asv_table_version & 0xF);
}
