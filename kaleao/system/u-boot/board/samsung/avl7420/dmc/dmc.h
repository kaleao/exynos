...
typedef void (*dmc_preset_func_t)(int ch);
...
typedef void (*dmc_io_odt_func_t)(int ch, int byte, int odt);
typedef void (*dmc_io_vref_func_t)(int ch, int byte, dmc_vref_t *vref);

typedef void (*dmc_dfi_init_func_t)(int ch, int init);
typedef int (*dmc_dfi_stat_func_t)(int ch);
typedef void (*dmc_dfi_update_func_t)(int ch);
...
typedef void (*dmc_cal_preset_func_t)(int ch);
...
typedef void (*dmc_cal_ca_func_t)(int ch, int(*clock)(int *), int freq);
...
typedef int (*dmc_cal_hwperiodic_func_t)(int ch, int periodic_en, int retrain_en, int num_rank);
typedef void (*dmc_cal_postset_func_t)(int ch);
...
typedef void (*dmc_srx_func_t)(unsigned char ch, unsigned char rank);
typedef int (*dmc_pd_func_t)(unsigned char ch, unsigned char rank);
...
typedef void (*dmc_mrr_func_t)(unsigned char ch, unsigned char rank, unsigned char ma);
...
typedef lp4_density_t (*dmc_detect_lp4_sizeinfo_func_t)(unsigned int ch, int cs, char mr);
typedef int (*dmc_set_by_density_func_t)(lp4_density_t density, u32 num_chip);
typedef int (*dmc_get_num_chip_func_t)(lp4_density_t density);
//typedef int (*dmc_command_func_t)(int command, ...);

typedef struct {
	dmc_reset_func_t reset;
	dmc_preset_func_t preset;
	dmc_done_func_t done;
	dmc_detect_lp4_sizeinfo_func_t sizeinfo;
	dmc_set_by_density_func_t setbydensity;
	dmc_get_num_chip_func_t getnumrank;
} dmc_init_func_t;
...
extern void InitDmcDriver(void);
extern int dmc_vtc_en(int ch);
extern void dmc_io_pd(int ch);
extern void dmc_low_power(int ch);
extern int clk_init_root_clock_gating(void);
extern void dmc_phy_upd_mode(int ch, int mode);
extern void dmc_sel_drex_timing_set(int ch, int set);
extern unsigned int get_chip_status();
extern unsigned int get_vref_fuse_bits(void);
#endif
