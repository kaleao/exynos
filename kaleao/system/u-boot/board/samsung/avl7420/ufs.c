...
int ufs_board_init(int host_index, struct ufs_host *ufs)
{
	...
	printf("UFS ufs.c: ufs_board_init()\n");
	set_ufs_clk(host_index);
	printf("UFS ufs.c: ufs_board_init() after set_ufs_clock()\n");
	err = exynos_pinmux_config(PERIPH_ID_UFS0, PINMUX_FLAG_NONE);
	printf("UFS ufs.c: ufs_board_init() after exynos_pinmux_config()\n");
	return 0;
}
