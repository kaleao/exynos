...
/* fsp-1 */
	...
	DmcDriver.command.mrw( ch, DMC_RANK_ALL, 14, dram_vref );			// VREF-DQ 19.2%
	__wait( ch, 1 * msec);
 ...
 *
 *****************************************************************************/
static void calibration_preset(int ch)
{
 ...
 *
 *****************************************************************************/
static void calibration_preset_gate_train_evt0(int ch)
{
//	DmcDriver.cal.preset( ch );
 ...
 *****************************************************************************/
#if 0
static void command_bus_training(int ch, int vref, int(*clock)(int *), int freq)
{
...
static void gate_training(int ch, int rank)
{
	DmcDriver.cal.gate( ch, rank );
 ...
 *
 *****************************************************************************/
static void read_dq_calibration(int ch, int rank)
{
 ...
 *
 *****************************************************************************/
static void write_dq_calibration(int ch, int rank)
{
 ...
 *
 *****************************************************************************/
static void memory_init_done(int ch, int resume)
{
	if( resume == 1 ) {
 ...
 *
 *****************************************************************************/
static void change_freq(int mega_hertz)
{
#if 0
...
int MemoryInit(int dmc_clk, int resume) /*(int resume, int(*clock)(int *), lp4_conf_t *conf)*/
{
	...
	int rank;
	int freq;
	lp4_density_t density;
	int num_rank;
	dmc_rank_t target_rank;
	int i;
...
//		calibration_preset( ch );
	...
	//1 Get Memory Density before Write Training
#if DRAM_SIZE_DETECT
	density = DmcDriver.init.sizeinfo(0, 0, 8);
	num_rank = DmcDriver.init.getnumrank(density);
#else
	density = LP4_6Gb_Die;
	num_rank = 2;
#endif

#ifdef CONFIG_DRAM_MULTI_CAPACITY_SUPPORT
	switch (density) {
		case LP4_6Gb_Die:
		case LP4_12Gb_Die:
			...
			break;
		case LP4_8Gb_Die:
			__REG64((u64)CONFIG_PHY_BL_SYS_INFO_DRAM_SIZE) = 0x100000000;
			break;
		default:
			__REG64((u64)CONFIG_PHY_BL_SYS_INFO_DRAM_SIZE) = 0xC0000000;
			break;
	...
	DmcDriver.init.setbydensity(density, num_rank);

	//1 Gate Training (rank1 if necessary)
	#if (DMC_DQS_TYPE == DMC_DQS_MODE1)
	if(2 == num_rank) {
		for( ch = 0; ch < NUM_DMC_CH; ch++ ) {
			/* For EVT0, modified pattern for gate train */
			//calibration_preset_gate_train_evt0( ch );
			gate_training( ch, DMC_RANK_1 );
		}
	...
	for( ch = 0; ch < NUM_DMC_CH; ch++ ) {
		...
		DmcDriver.cal.hw_periodic(ch, 1, 0, num_rank);
		#else
		DmcDriver.cal.hw_periodic(ch, 0, 0, num_rank);
		#endif
...
#ifndef CONFIG_DRAM_MULTI_CAPACITY_SUPPORT
