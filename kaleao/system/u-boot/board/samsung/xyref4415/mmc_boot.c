...
#define EMMCSWRST_FIRST_USB_SECOND_DEV		0x0
#define EMMC_FIRST_USB_SECOND_DEV		0x8
...
static int find_second_boot_dev(void)
{
	...
	if (om_status == SDMMC_SECOND_DEV)
		return BOOT_SEC_DEV;
	else if (om_status == EMMCSWRST_FIRST_USB_SECOND_DEV)
		return BOOT_USB;
	else if (om_status == EMMC_FIRST_USB_SECOND_DEV)
		return BOOT_USB;
	else
	    return -1;
}
