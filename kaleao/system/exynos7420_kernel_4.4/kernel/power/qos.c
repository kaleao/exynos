...
int pm_qos_update_target_7420(struct pm_qos_constraints *c, struct plist_node *node,
                         enum pm_qos_req_action action, int value)
{
        ...
        if ((c == cluster1_max_const || c == cluster0_max_const) &&
                ...
                value = c->default_value;
#endif
	pr_info("!!!check pm_qos_get_value!!!\n");
        prev_value = pm_qos_get_value(c);
	pr_info("!!!check pm_qos_get_value ok!!!\n");
        if (value == PM_QOS_DEFAULT_VALUE)
                new_value = c->default_value;
        else{
               new_value = value;
	}
	pr_info("!!!!try switch the value!!\n");
        switch (action) {
 ...
 * @work: work struct for the delPM_QOS_CLUSTER1_FREQ_MIN,ayed work (timeout)
 *
			...
			int pm_qos_class, s32 value)
{
	...
	trace_pm_qos_add_request(pm_qos_class, value);
        //cperakis error here
	pr_info("!!!pm_qos_class value is %d!!!\n",pm_qos_class);
	pm_qos_update_target_7420(pm_qos_array[pm_qos_class]->constraints,
