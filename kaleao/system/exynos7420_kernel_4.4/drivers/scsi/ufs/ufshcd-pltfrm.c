		       ...
		       const struct ufshcd_drv_data *drv_data)
{
	...
	/* mem_res->start = 0x15570000, mem_res->end = 0x155700FF */
	mem_res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	//printk("%s : mem_res->start = 0x%08X mem_res->end = 0x%08X  <======\n", __func__, (unsigned int)mem_res->start, (unsigned int)mem_res->end);
	...
	err = ufshcd_init(dev, &hba, mmio_base, irq, drv_data);
	if (err) {
