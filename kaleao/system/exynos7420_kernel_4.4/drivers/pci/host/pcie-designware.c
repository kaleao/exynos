...
static int dw_msi_setup_irq(struct msi_controller *chip, struct pci_dev *pdev, struct msi_desc *desc);
static int dw_msi_setup_irqs(struct msi_controller *chip, struct pci_dev *pdev, int nvec, int type);
...
void dw_pcie_msi_init(struct pcie_port *pp)
{
	...
	pp->msi_data = __get_free_pages(GFP_DMA32, 0);
	msi_target = virt_to_phys((void *)pp->msi_data);
	
	//printk(KERN_INFO "DEBUG: MSI: %p\n", msi_target);
...
static void dw_msi_setup_msg(struct pcie_port *pp, unsigned int irq, u32 pos)
{
	...
	//printk(KERN_INFO "DEBUG: MSI_msg: %p\n", msi_target);
			     ...
			     int nvec, int type)
{
	...
	{
		status = dw_msi_setup_irq(chip, pdev, desc);
		if(status!=0)
		{
			return status;
		}
			...
			irq_hw_number_t hwirq)
{
	...
	irq_set_chip_and_handler(irq, &dw_msi_chip, handle_simple_irq);
...
void dw_pcie_prog_viewport_cfg0(struct pcie_port *pp, u32 busdev)
{
        ...
        dw_pcie_wr_own_conf(pp, PCIE_ATU_LIMIT, 4,  pp->cfg0_base + pp->cfg0_size - 1);
        ...
        dw_pcie_rd_own_conf(pp, PCIE_ATU_CR2, 4, &val);
}
...
static void dw_pcie_prog_viewport_cfg1(struct pcie_port *pp, u32 busdev)
{
        ...
        dw_pcie_wr_own_conf(pp, PCIE_ATU_LIMIT, 4, pp->cfg1_base + pp->cfg1_size - 1);
        dw_pcie_wr_own_conf(pp, PCIE_ATU_LOWER_TARGET, 4,  busdev);
        dw_pcie_wr_own_conf(pp, PCIE_ATU_UPPER_TARGET, 4, 0);
        dw_pcie_rd_own_conf(pp, PCIE_ATU_CR2, 4, &val);
}
...
void dw_pcie_prog_viewport_mem_outbound(struct pcie_port *pp)
{
        ...
        dw_pcie_wr_own_conf(pp, PCIE_ATU_LIMIT, 4, pp->mem_base + pp->mem_size - 1);
...
static void dw_pcie_prog_viewport_io_outbound(struct pcie_port *pp)
{
        ...
        dw_pcie_wr_own_conf(pp, PCIE_ATU_LIMIT, 4, pp->io_base + pp->io_size - 1);
...
//BABIS: Changed implementation of rd/wr other_conf
...
static int dw_pcie_rd_conf_ext(struct pci_bus *bus, u32 devfn, int where,
			int size, u32 *val)
{
	struct pcie_port *pp = bus->sysdata;
	int status;
	...
	spin_lock_irqsave(&pp->conf_lock, flags);
	status = dw_pcie_rd_conf(bus, devfn, where, size, val);
	spin_unlock_irqrestore(&pp->conf_lock, flags);
	
	return status;
}

static int dw_pcie_wr_conf_ext(struct pci_bus *bus, u32 devfn,
			int where, int size, u32 val)
{
	struct pcie_port *pp = bus->sysdata;
	int status;
	...
	spin_lock_irqsave(&pp->conf_lock, flags);
	status = dw_pcie_wr_conf(bus, devfn, where, size, val);	
	spin_unlock_irqrestore(&pp->conf_lock, flags);
	
	return status;
}

void dw_pcie_restore_msi(struct pcie_port *pp)
{	uint64_t msi_target;
	uint32_t val;
	unsigned long flags;
	...
	dw_pcie_rd_own_conf(pp, PCIE_MSI_INTR0_ENABLE, 4, &val);
	//val |= 0x1ff;
	val = 0x6;
	dw_pcie_wr_own_conf(pp, PCIE_MSI_INTR0_ENABLE, 4, val);
	spin_unlock_irqrestore(&pp->conf_lock, flags);
}

static struct pci_ops dw_pcie_ops = {
	.read = dw_pcie_rd_conf_ext,
	.write = dw_pcie_wr_conf_ext,
};
